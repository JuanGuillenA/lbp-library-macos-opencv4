...
> **Nota:** Este repositorio contiene el código de la librería original de LBP Library. No es código de mi autoría. Los cambios incluidos aquí se hicieron con fines académicos para que funcione en mi entorno macOS con OpenCV 4.x.
>
> Los ajustes realizados son específicos para la compilación en mi equipo Mac y no representan una reescritura del proyecto original.

## Cambios realizados para macOS
- Actualicé `CMakeLists.txt` para usar `cmake_minimum_required(VERSION 3.5)` en lugar de la versión obsoleta 2.8.
- Corregí `package_lbp/elbp/ELBP.cpp` cambiando `dst.at<unsigned int>(...)` por `dst.at<int>(...)` para que el acceso al `cv::Mat` con tipo `CV_32SC1` compile correctamente en OpenCV 4.x.

## Dependencias
- macOS
- OpenCV 4.x instalado (por ejemplo, vía Homebrew)
- CMake 3.5 o superior
- Compilador C++ compatible con AppleClang / GCC
...
./build/lbp
```
3. Si todo está correcto, se abrirán ventanas de OpenCV con la cámara o la prueba de video del programa.
